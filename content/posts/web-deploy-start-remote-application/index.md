...
msdeploy -verb:sync -source:recycleApp -dest:recycleApp="",recycleMode="StartAppPool", computerName=

![1.png](1.png)

如果要透過 Web Management Service 去做遠端電腦連線的話，則 dest 這邊要使用 wmsvc provider setting 去指定遠端電腦的位置。

msdeploy -verb:sync -source:recycleApp -dest:recycleApp="",recycleMode="StartAppPool",wmsvc=,userName=,password= -allowUntrusted
