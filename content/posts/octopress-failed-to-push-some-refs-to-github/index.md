---
title: "Octopress - Failed to push some refs to GitHub"
date: "2016-05-11 22:05:00"
description: "今天在使用 rake deploy 試圖發佈文章時，在發佈到 Github 這步驟時發生了錯誤，發佈的動作被 rejected 了。 要解決這樣的問題我們可以依下面步驟操作。 就像下面這樣： 操作完就重新建立靜態檔案並 Deploy。 沒意外的話就能正確的發佈。"
tags: [Octopress]
---

今天在使用 rake deploy 試圖發佈文章時，在發佈到 Github 這步驟時發生了錯誤，發佈的動作被 rejected 了。

![1.png](/images/posts/OctopressDeployFail/1.png)

要解決這樣的問題我們可以依下面步驟操作。

```bash
cd _deploy
git reset --hard origin/master
cd ..
```

就像下面這樣：

![2.png](/images/posts/OctopressDeployFail/2.png)

操作完就重新建立靜態檔案並 Deploy。

```bash
rake generate
rake deploy
```

沒意外的話就能正確的發佈。

![3.png](/images/posts/OctopressDeployFail/3.png)

Link
----
* [ruby - Octopress pushing error to GitHub - Stack Overflow](http://stackoverflow.com/questions/19619280/octopress-pushing-error-to-github)