---
title: "LogDevice - Removing log using ldshell"
date: "2018-10-10 23:31:32"
description: "要使用 ldshell 刪除 LogDevice 的 Log，可以使用 -c 參數指定使用 Interactive Mode，帶入 LogDevice 的設定檔，帶入 logs remove 及指定要刪除的 Log 即可。 ldshell -c logs remove Link"
tags: [LogDevice]
---

要使用 ldshell 刪除 LogDevice 的 Log，可以使用 -c 參數指定使用 Interactive Mode，帶入  LogDevice 的設定檔，帶入 logs remove 及指定要刪除的 Log 即可。

ldshell -c  logs remove

![1.png](1.png)

Link