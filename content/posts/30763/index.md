...
String pool簡單來說就是一個HashTable,其Key值是字串內容，Value是物件實體的位置。其內容值會在程式編譯時期做初始設定，將程式碼中用到的字串加入。因此不同字串變數所存放的靜態字串，若是是一樣的字串，字串會在編譯時期加入String pool，透過String pool的協助兩個變數會指到相同的物件實體。              string str1 = "abcd1234";
