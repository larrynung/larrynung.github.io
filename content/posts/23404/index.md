...
 
  可以看到建構Flickr時要帶入的不外乎就是apiKey、secret、與token。apiKey與secret可參閱Flickr開發系列 - 應用程式與API Key的申請這篇取得，多半的動作我們只需要戴入apiKey與secret，當有進階的處理動作才須帶入token。  
...
loginDlg.Width = 500;      
loginDlg.Height = 600;      
loginDlg.StartPosition = FormStartPosition.CenterScreen;  WebBrowser browser = new WebBrowser();     
browser.Navigate(url);      
browser.Dock = DockStyle.Fill;  loginDlg.Controls.Add(browser);     
loginDlg.ShowDialog();  
...
## Link
     Flickr.Net API Library     [Windows Mobile .NET CF] 拍照上傳 Flickr – Day2     用 Flickr.NET 來上傳照片的開發步驟